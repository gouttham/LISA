# xview2 metrics, modified from https://github.com/DIUx-xView/xView2_scoring/blob/master/xview2_metrics.py
# 
...
NEW_PRED_DIR = "./predictions_tmp/"
NEW_TARGET_DIR = "./targets_tmp/"
...
class PathHandler:
    def __init__(self, pred_dir:Path, targ_dir:Path, img_id:str, test_hold:str):
        ...
        self.lp = pred_dir/f"{test_hold}_localization_{img_id}_prediction.jpg" # localization prediction
        self.dp = pred_dir/f"{test_hold}_damage_{img_id}_prediction.jpg" # damage prediction
...
class XviewMetrics:
    ...
    @classmethod
    def compute_score(cls, pred_dir=NEW_PRED_DIR, targ_dir=NEW_TARGET_DIR, out_fp="res.json"):
        """
        Args:
...
def prepare_dirs(target_dir_from_xbd, pred_dir):    
    in_filenames = glob.glob(pred_dir + "*_mask_0.jpg")
    in_filenames = [fn[len(pred_dir):] for fn in in_filenames]
    in_filenames = [fn[:-11] for fn in in_filenames]
    in_filenames = sorted(list(set([(fn.split("_")[0] + "_" + fn.split("_")[1]) for fn in in_filenames])))

    for i, file in enumerate(in_filenames):
        lp_fn = pred_dir + file + "_pre_disaster_mask_0.jpg"
        dp_fn = pred_dir + file + "_post_disaster_mask_0.jpg"
        lt_fn = target_dir_from_xbd + file + "_pre_disaster_target.png"
        dt_fn = target_dir_from_xbd + file + "_post_disaster_target.png"
        check_is_present = True
        for p in [lp_fn, dp_fn, lt_fn, dt_fn]:
            check_is_present = check_is_present and os.path.isfile(p)
        if check_is_present:
            shutil.copy(lp_fn, NEW_PRED_DIR + "test_localization_" + str(i) + "_prediction.jpg")
            shutil.copy(dp_fn, NEW_PRED_DIR + "test_damage_" + str(i) + "_prediction.jpg")
            shutil.copy(lt_fn, NEW_TARGET_DIR + "test_localization_" + str(i) + "_target.png")
            shutil.copy(dt_fn, NEW_TARGET_DIR + "test_damage_" + str(i) + "_target.png")
             
def del_temp_files():
    shutil.rmtree(NEW_PRED_DIR)
    shutil.rmtree(NEW_TARGET_DIR)

if __name__ == '__main__':
    assert len(sys.argv) >= 3, f"Please provide arguments like this:\n python3 xview2_metrics.py [dir of image targets downloaded from xview2] [dir of your predictions]"

    os.mkdir(NEW_PRED_DIR)
    os.mkdir(NEW_TARGET_DIR)
    prepare_dirs(sys.argv[1], sys.argv[2])
    XviewMetrics(NEW_PRED_DIR, NEW_TARGET_DIR).compute_score()
    del_temp_files()