...
from .cd_dataset import CD_Dataset,Contrastive_CD_Dataset
...
def collate_fn3(
    ...
    if isinstance(images_list[0], tuple):
        new_images_list = []
        for ech_im in images_list:
            new_images_list.append(torch.stack([ech_im[0], ech_im[1]], dim=0))

        new_images_clip_list = []
        for ech_cl in images_clip_list:
            new_images_clip_list.append(torch.stack([ech_cl[0], ech_cl[1]], dim=0))

        images_list = new_images_list
        images_clip_list = new_images_clip_list
...
class collate_fn2(object):
    ...
    def __call__(self, batch):
        ...
        for conversation, target in zip(conversation_list, targets):
            ...
            if False:
                z = target.clone()
                z = torch.where(z == IGNORE_INDEX, self.tokenizer.unk_token_id, z)
                if local_rank == 0:
                    print(
                        "conversation: ",
                        conversation,
...
class HybridDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        for dataset in self.datasets:
            if dataset == "contrastive_cd_dataset":
                self.all_datasets.append(
                    Contrastive_CD_Dataset(
                        base_image_dir,
                        tokenizer,
                        vision_tower,
                        samples_per_epoch,
